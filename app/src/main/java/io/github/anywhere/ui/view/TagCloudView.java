...
import io.github.anywhere.ui.utils.TagCloud;

public class TagCloudView extends RelativeLayout {
	RelativeLayout navigation_bar;
	TextView mTextView1;
	public TagCloudView(Context mContext, int width, int height, List<Tag> tagList) {
		this(mContext, width, height, tagList, 30 , 54, 5 ); //default for min/max text size
	}
	public TagCloudView(Context mContext, int width, int height, List<Tag> tagList,
		...
		float[] tempColor1 = {0.1412f,0.7686f,0.87f,1}; //rgb Alpha
										//{1f,0f,0f,1}  red       {0.3882f,0.21568f,0.0f,1} orange
										//{0.9412f,0.7686f,0.2f,1} light orange
		float[] tempColor2 = {0f,0f,0f,1}; //rgb Alpha
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			float dx = x - centerX  + 10.5f;
			float dy = y - centerY + 10.5f;
	...
	OnClickListener OnTagClickListener(final String url){
		return new OnClickListener(){
			...
			public void onClick(View v) {
				//we now havne url from main code
