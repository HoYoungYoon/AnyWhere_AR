...
public class TagCloud implements Iterable{
	...
	public void create(boolean distrEven){
		...
		for (int i=0; i< tagCloud.size(); i++){
			...
			Log.i("Text Color Info :  " , tempColor[0] + " , " + tempColor[1] + " , " +tempColor[2] + " , ");
	...
	private void position(boolean distrEven, Tag newTag){
		...
		newTag.setLocX((int)(radius * Math.cos(theta)* 1.5f * Math.sin(phi)));
		newTag.setLocY((int)(radius * Math.sin(theta) * 1.5f * Math.sin(phi)));
	...
	private float[] getColorFromGradient(float perc){
        ...
        Log.i("TagColor RGB 1 :" , tagColor1[0] + " , "  +tagColor1[1] + " , "  +tagColor1[2]);
        Log.i("TagColor RGB 2 :" , tagColor2[0] + " , "  +tagColor2[1] + " , "  +tagColor2[2]);
	...
	private static final int TEXT_SIZE_MAX = 50 , TEXT_SIZE_MIN= 10;
	private static final float[] DEFAULT_COLOR1= { 0.186f, 0.125f, 0.988f, 1f};
	private static final float[] DEFAULT_COLOR2= { 0.1f, 0.1f,  0.1f, 1f};
