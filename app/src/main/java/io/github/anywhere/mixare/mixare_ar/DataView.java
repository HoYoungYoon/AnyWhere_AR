...
import io.github.anywhere.mixare.data.DataHandler;
import io.github.anywhere.mixare.data.DataSource;
import io.github.anywhere.mixare.gui.PaintScreen;
import io.github.anywhere.mixare.gui.RadarPoints;
import io.github.anywhere.mixare.gui.ScreenLine;
import io.github.anywhere.mixare.render.Camera;
...
public class DataView {
    ...
    private float radius = 0.1f;    // 검색 반경
    ...
    //37.700316, 127.195658DataSource.DATASOURCE datasource

    /**
     * 생성자
     */
    public DataView(MixContext ctx , ArrayList<DataModel> dataModels) {
        this.mixContext = ctx;
        this.Data_List = dataModels;
        for(int i = 0; i<dataModels.size(); i++) {
            String title = Data_List.get(i).getName();
            int likeCnt = Data_List.get(i).getLike_cnt();
            double lat = Data_List.get(i).getLat();
            double lon = Data_List.get(i).getLon();
            double alt = Data_List.get(i).getAltitude();
            String logo = Data_List.get(i).getLogo();
            tempMarkers.add(new Marker(title, likeCnt, lat, lon, alt, logo));
        }
...
/*
        if (ActivityCompat.checkSelfPermission(mixContext, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
        ...
        result.setResultCallback(new ResultCallback<PlaceLikelihoodBuffer>() {
            ...
            public void onResult(@NonNull PlaceLikelihoodBuffer likelyPlaces) {
                ...
                for (PlaceLikelihood placeLikelihood : likelyPlaces) {
                    ...
                    marker[i] = new Marker(mLikelyPlaceNames[i] ,mLikelyPlaceLatLngs[i].latitude ,
                            mLikelyPlaceLatLngs[i].longitude , curFix.getAltitude() ,  mLikelyPlaceAddresses[i] ) {
                        @Override
    ...
    public void requestData(String url, DataSource.DATAFORMAT dataformat, DataSource.DATASOURCE datasource) {
        ...
        request.url = url;
        Log.i(MixView.TAG, "Request Data " + request.url + " format :" + request.format);
        // 완성된 요청을 다운로더에 제출한다
    ...
    public void draw(PaintScreen dw) {
        ...
        if (state.nextLStatus == MixState.NOT_STARTED && !frozen) {
            ...
            if (mixContext.getStartUrl().length() > 0) {
                ...
                requestData(mixContext.getStartUrl(), DataSource.DATAFORMAT.MIXARE, DataSource.DATASOURCE.OWNURL);
            ...
            while ((dRes = dm.getNextResult()) != null) {
                ...
                if (!dRes.error) {
                  ...
                  //  for(int i = 0; i<mMaxEntries;i++)
					...
					dataHandler.addMarkers(tempMarkers);
		...
		/* 마커 업데이트 */
		dataHandler.updateActivationStatus(mixContext , curFix);	// 활성화 상태를 갱신
		// 각각의 마커에 적용 
		for (int i = dataHandler.getMarkerCount()-1; i >= 0; i--) {
			Marker ma = dataHandler.getMarker(i);
            //Log.v(MixView.TAG , "Draw Marker Count:  "+ ma.isActive() + "  ,  " + ma.getDistance() /1000f +"");
