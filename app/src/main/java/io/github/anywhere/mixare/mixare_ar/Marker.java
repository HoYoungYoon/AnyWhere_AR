...
import io.github.anywhere.mixare.gui.ScreenObj;
import io.github.anywhere.mixare.gui.TextObj;
import io.github.anywhere.mixare.reality.PhysicalPlace;
import io.github.anywhere.mixare.render.Camera;
import io.github.anywhere.mixare.render.MixVector;
...
//	private MixVector oMarker = new MixVector();
	...
	public Marker(String title, int like_cnt , double latitude, double longitude, double altitude, String logo) {
		super();
		...
		this.mGeoLoc = new PhysicalPlace(latitude,longitude,altitude);
			...
			URL = "webpage:" + URLDecoder.decode("http://anywhere9.dothome.co.kr/php/page_move.php?id="+title);
...
//	}
	...
	public void drawCircle(PaintScreen dw) {
		...
		if (isVisible) {
			...
			dw.paintCircle(cMarker.x, cMarker.y, 100.0f);
		}
	}
	...
	public void drawTextBlock(PaintScreen dw) {
		...
		// 텍스트 블록(텍스트 오브젝트) 생성
		textBlock = new TextObj(textStr,like_cnt , logo , Math.round(maxHeight / 3f) +0.3f,
				250, dw, underline);

		// 출력되는 상황일 경우
		if (isVisible) {
			// 데이터 소스에 따른 컬러를 지정
			dw.setColor(DataSource.getColor());
